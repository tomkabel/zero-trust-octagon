...
The in-flight buffer has finite capacity. Three failure modes must be handled at architecture level:

1. **Buffer overflow:** If the in-flight request volume at graft time exceeds buffer capacity (a burst of 1,000 concurrent requests), the enforcement point must drop the oldest in-flight requests rather than rejecting new ones — a dropped request produces a connection error, which an attacker can distinguish from a successful graft transition. The mitigation is not larger buffers; it is an abort-and-graft strategy: terminate all in-flight requests immediately with a synthetic transport-level error (connection reset), then begin serving garden data on retry. This trades a detectable transport glitch for a guaranteed clean application-layer transition. The transport glitch is indistinguishable from a transient network error in any production environment and produces no cross-request inconsistency.

2. **Persistent connections (WebSocket, SSE, HTTP/2):** Long-lived connections multiplex multiple logical operations over a single transport. Grafting mid-stream on a persistent connection requires either (a) terminating the transport and forcing reconnection — the abort-and-graft strategy applied to the connection layer — or (b) performing the graft at the logical stream boundary within the multiplexed connection. Option (b) is architecturally cleaner but requires protocol-aware graft logic in the enforcement point. Option (a) is simpler and consistent with the transport-glitch strategy for HTTP/1.1.

3. **Buffer drain timeout:** If in-flight requests are long-running (database queries, batch processing), the buffer holds them until completion. A timeout bound must exist: requests that do not complete within the grace period are terminated and must be retried by the client. The attacker receives an error — indistinguishable from a production timeout — and retries against the garden.

The architectural trade-off is explicit: protocol-level graft transparency (no glitch detectable at the application layer) vs. buffer complexity. The abort-and-graft strategy accepts a transport-level glitch that is cryptographically indistinguishable from ambient network noise. A transport-level glitch occurs hundreds of times per day in any production environment and provides no signal to the attacker. A cross-request data inconsistency — one response from production, the next from the garden — occurs zero times per day and is a definitive signal the moment it happens. The architecture chooses transport glitches over data leaks every time.
...
An attacker who obtains two sessions under different identities and cross-references data between them must not receive identical synthetic data. This is the cross-session consistency problem: the garden must differentiate *per-attacker-identity* without leaking that differentiation as a detection signal.

The garden maintains a mapping function $M: (identity, resource\_id) \rightarrow synthetic\_record$. If the attacker compromises identity $I_1$ and later identity $I_2$, both requesting "customer #482," the mapping function returns two different synthetic records. This is necessary: if both identities received the same synthetic customer, the attacker who already exfiltrated under $I_1$ would recognize the data as synthetic when seen again under $I_2$. The mapping function is deterministic per identity — the same identity requesting the same resource always gets the same synthetic record (within-session consistency) — but produces different synthetic records across identities.

The cross-session consistency constraint extends to related records: if $I_1$ exfiltrated synthetic customer #482 with an associated transaction history, and $I_2$ later queries a transaction that references customer #482, the garden must honor the referential integrity of $I_2$'s synthetic world without leaking data from $I_1$'s. Implementation strategy: each attacker identity maps to an independent synthetic data universe with its own seed. The universes are structurally identical (same schema, same data types, same field lengths) but content-distinct (different values at every record). An attacker seeing data from two sessions sees two plausibly real but unrelated datasets — not the same dataset twice.

The engineering cost of maintaining per-identity synthetic universes grows linearly with attacker identity count. For high-volume attack scenarios (credential stuffing yielding thousands of compromised identities), the garden may collapse identities sharing common behavioral fingerprints (same originating IP range, same TTP profile, same temporal window) into a shared synthetic universe to bound storage and generation cost.
...
An attacker aware of Trickle-Truth can game the satiation criterion by alternating between high-value TTPs and filler — injecting novel techniques periodically to keep the discovery rate above threshold while primarily exfiltrating garden data. The defense requires a **combined termination criterion** that gates on both intelligence yield and session resource cost:

- **Primary criterion:** New TTP discovery rate below threshold $\tau_{ttp}$ for a minimum consecutive interval $t_{min}$ — prevents gaming via intermittent novelty injection.
- **Secondary criterion:** Total session duration exceeds $T_{max}$ — an absolute time cap prevents indefinite garden operation regardless of intelligence yield.
- **Tertiary criterion (cost-bound):** Cumulative LLM inference cost for the session exceeds $C_{max}$ — ensures the deception's resource cost does not exceed its intelligence value, even if the attacker is still producing new TTPs.

The three criteria together prevent the attacker from keeping the garden alive by injecting just enough novelty. The defender terminates the session when: (a) intelligence yield has demonstrably peaked AND a minimum observation window has elapsed, OR (b) the session has exceeded its time budget, OR (c) the session has exceeded its cost budget. Any one condition satisfied triggers de-grafting. An attacker who spaces 6 genuinely novel TTPs across a 14-hour session exceeds $T_{max}$ and is terminated regardless of intelligence value — the architecture accepts that some intelligence may be left on the table in exchange for bounded operational cost.

The values of $\tau_{ttp}$, $t_{min}$, $T_{max}$, and $C_{max}$ are domain-calibrated parameters, not architectural constants. Organizations tuning these parameters should begin with conservative values (short $T_{max}$, low $C_{max}$) and expand as operational experience validates the garden's effectiveness and cost profile.
...
4. **Post-deception analysis:** The full session recording — every request, every response, every TTP extracted — is packaged for the defender's threat intelligence pipeline.

**Caveat on TTP extraction:** TTPs extracted in the garden measure attacker behavior against *synthetic* data, not real data. An attacker exfiltrating garden records uses the same tooling, same command sequences, and same lateral movement patterns they would use against production — but the triggers for those behaviors are generated by the garden, not by the defender's real infrastructure. A TTP observed in the garden (e.g., "attacker ran Mimikatz variant X against synthetic service account Y") is a valid observation of the attacker's *capability* but not a direct observation of their *target selection* against real infrastructure. The intelligence value is the capability catalog: what tools the attacker possesses, what credential types they prioritize, what exfiltration channels they prefer. The intelligence deficit is the target priority model: which real assets the attacker would have pursued given real data. This distinction must be preserved in threat intelligence reporting — garden-extracted TTPs are annotated with `data_domain: synthetic` in the threat intelligence pipeline to prevent analysts from treating synthetic-target behavior as real-target intent.

The de-grafting protocol gives the attacker no *protocol-level* indication of garden presence: no account lockout, no 403, no captcha, no redirect, no behavioral tripwire — only a generic "service unavailable" error indistinguishable from a production outage. This is what "no indication" means: the architecture does not produce a signal the attacker can attribute to deception rather than infrastructure failure.

The attacker may still infer garden presence through *data-level* analysis: statistical artifacts in LLM-generated content, coherence failures in long sequences, or distributional patterns that differ from production data (see prior discussion in Garden Environment). This distinction is architecturally fundamental. Protocol-level signals are produced by the infrastructure and are under the defender's control. Data-level signals are produced by the generation pipeline and are subject to the known limits of LLM fidelity. The de-grafting protocol eliminates all protocol-level signals. Data-level signal suppression is a continuous improvement problem for the garden generation pipeline, not a binary design goal.
...
> **Caveat:** $A_{D9}$ weights all events equally regardless of severity. An organization can achieve $A_{D9}=0.95$ by automating all low-severity alerts while leaving critical alerts manual. Always evaluate $A_{D9}$ alongside the severity distribution of events that still require human response.
...
- **Small Rotation ($D_{D9} = 2$):** Two people. If one is unavailable, the other covers. MTTR variance decreases. This is *statistical* redundancy, not *structural* redundancy — both responders share the same alert pipeline, the same tooling, and possibly the same failure modes (both may be unavailable during the same overnight window, both may be on the same flight, both may burn out simultaneously). Small rotation cuts MTTR variance by roughly half but does not eliminate the bimodal distribution.
...
**D9c: Response Time Variance ($\sigma_{TTR}$)**

The standard deviation of individual response times across all incidents.

$$\sigma_{TTR} = \sqrt{\frac{1}{n}\sum_{i=1}^{n}(TTR_i - \overline{TTR})^2}$$

- **Single ($\sigma_{TTR}$ high):** TTR ranges from 3 minutes (Pat at desk) to 25+ minutes (Pat unavailable). Average TTR is meaningless — it obscures the structural risk.
- **Small Rotation ($\sigma_{TTR}$ moderate):** Variance narrows with redundancy but still spikes during shift transitions.
- **Enterprise SOC ($\sigma_{TTR}$ low):** Consistent response time regardless of time of day or personnel availability.
- **Fully Automated ($\sigma_{TTR}$ near-zero):** Response time is bounded by the automation pipeline, not human availability.
...
3. What is your $\sigma_{TTR}$ over the last 12 months? Because TTR distributions are often bimodal, use percentiles (e.g., P50 vs P95) rather than mean-dependent thresholds. A large gap between P50 and P95 is a reliable signal of a human continuity problem.
